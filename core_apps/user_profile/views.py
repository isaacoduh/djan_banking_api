...
class ProfileDetailAPIView(generics.RetrieveUpdateAPIView):
    ...
    def record_profile_view(self, profile: Profile) -> None:
        ...
        obj, created = ContentView.objects.update_or_create(content_type=content_type, object_id=profile.id, user=user, viewer_ip=viewer_ip, defaults={"last_viewed": timezone.now(),})
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if updated_instance.is_complete_with_next_of_kin():
                    existing_account = BankAccount.objects.filter(user=request.user, currency=updated_instance.account_currency, account_type=updated_instance.account_type).first()

                    if not existing_account:
                        bank_account = create_bank_account(request.user, currency=updated_instance.account_currency, account_type=updated_instance.account_type,)
                        message = ("Profile updated and new bank account created successfully. An email "
...
class NextOfKinDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request: Response, *args: Any, **kwargs: Any) -> Response:
        partial = kwargs.pop("partial", False)
        instance = self.get_object()
        serializer = self.get_serializer(instance, data=request.data, partial=partial)
        serializer.is_valid(raise_exception=True)
