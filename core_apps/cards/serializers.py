...
class VirtualCardCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self,attrs):
        ...
        if user.virtual_cards.count() >= 3:
            raise serializers.ValidationError("Your can only have up to 3 virtual cards at a time")
        return attrs
    ...
    def create(self, validated_data) -> dict:
        ...
        virtual_card = VirtualCard.objects.create(user=user, bank_account=bank_account, card_number=card_number, expiry_date=expiry_date, cvv=cvv)
