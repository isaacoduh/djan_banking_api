...
class VirtualCardListCreateAPIView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs) -> Response:
        if request.user.virtual_cards.count() >= 3:
            return Response(
                {"error": "Your can only have up to 3 virtual cards at a time."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class VirtualCardDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        if obj.user != self.request.user:
            raise PermissionDenied("You cannot perform this action, because the card does not belong to you!")
        raise obj
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            if instance.balance > 0:
                return Response({"error": "Cannot delete a card with non-zero balance"}, status=status.HTTP_400_BAD_REQUEST)
            self.perform_destroy(instance)
            logger.info(f"Visa card number {instance.card_number}, balonging to {instance.user.full_name} destroyed")
            return Response({"message": "Card successfully deleted!"}, status=status.HTTP_200_OK)
            ...
            return Response({"error": "An unexpected error occured while deleting this card!"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class VirtualCardTopUpAPIView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({"error": f"The amount {amount} is not valid"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if bank_account.account_balance < amount:
            return Response({"error": "Insufficient funds in the bank account."}, status=status.HTTP_400_BAD_REQUEST)
