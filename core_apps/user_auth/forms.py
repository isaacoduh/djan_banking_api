...
class UserCreationForm(DjangoUserCreationForm):
    class Meta:
        model = User
        fields = ["email", "id_no", "first_name", "middle_name", "last_name", "security_question","security_answer","is_staff","is_superuser"]
    ...
    def clean_id_no(self):
        ...
        if User.objects.filter(id_no=id_no).exists():
            raise ValidationError(_("A user with that ID already exists."))
