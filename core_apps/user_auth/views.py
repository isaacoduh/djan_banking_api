...
class CustomTokenCreateView(TokenCreateView):
    def _action(self, serializer):
        ...
        if user.is_locked_out:
            return Response(
                {"error": f"Account is locked due to multiple failed login attempts. Please try again after {settings.LOCKOUT_DURATION.total_seconds() / 60 } minutes. " }, status=status.HTTP_403_FORBIDDEN
            )
        ...
        return Response({"success": "OTP send to your email", "email": user.email}, status=status.HTTP_200_OK)
    ...
    def post(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        try:
            ...
            if user:
                ...
                if failed_attempts >= setings.LOGIN_ATTEMPTS:
                    return Response({"error": f"You have exceeded the maximum number of login attemps. Your account has been locked for {settings.LOCKOUT_DURATION.total_seconds() / 60} minutes. An email has been sent to you with further instructions" }, status=status.HTTP_403_FORBIDDEN)
            else:
                logger.error(f"Failed login attempt for non-existent user: {email}")
            
            return Response({"error": "Your login credentials are not correct"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        if refresh_res.status_code == status.HTTP_200_OK:
            ...
            if access_token and refresh_token:
                ...
                logger.error("Access or refresh token not found in refresh")
        return refresh_res
...
class OTPVerifyView(APIView):
    ...
    def post(self, request):
        ...
        if user.is_locked_out:
            return Response({"error": f"Account is locked due to multiple failed logged in attempts. Please try again after f{settings.LOCKOUT_DURATION.total_seconds() / 60 } minutes "}, status=status.HTTP_403_FORBIDDEN,)
        ...
        response = Response({"success": "Login successful. Now add your profile information, so that we can create an account for you."}, status=status.HTTP_200_OK)
