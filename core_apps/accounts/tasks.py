...
def generate_transaction_pdf(user_id, start_date, end_date, account_number=None):
    try:
        ...
        transactions = Transaction.objects.filter(Q(sender=user) | Q(receiver=user), created_at__date__range=[start_date, end_date],).order_by("-created_at")
        ...
        elements.append(Paragraph(f"Transaction History from ({start_date} to {end_date})", styles["Title"],))
        ...
        message = (f"Dear {user.full_name}, Please find attached your transaction history PDF")
        from_email = settings.DEFAULT_FROM_EMAIL
        recipient_list = [user.email]
        email = EmailMessage(subject, message, from_email, recipient_list)
        email.attach(f"Transactions_{start_date}_to_{end_date}.pdf", pdf, "application/pdf")
        ...
        try:
            ...
            logger.error(f"Failed to send transaction history pdf to {email}: Error {str(e)}")
    except Exception as e:
...
def apply_daily_interest():
    savings_account = BankAccount.objects.filter(account_type = BankAccount.AccountType.SAVINGS)
    ...
    return f"Applied daily interest {savings_account.count()} savings account"
...
def detect_suspicious_activities():
    ...
    large_transactions = Transaction.objects.filter(amount__gte=LARGE_TRANSACTION_THRESHOLD, created_at__lte=time_threshold)
    
    for transaction in large_transactions:
        suspicious_activities.append(f"Large transaction detected: {transaction.amount} by user {transaction.user.email}")
    ...
    for user in users:
        ...
        if transaction_count >= FREQUENT_TRANSACTION_THRESHOLD:
            suspicious_activities.append(f"Frequest transactions detected: {transaction_count} by user {user.email} ")
    ...
    for account in accounts:
        balance_change = Transaction.objects.filter(
            ...
            created_at__gte=time_threshold).aggregate(
        ...
        if abs(total_change) > LARGE_TRANSACTION_THRESHOLD:
            suspicious_activities.append(f"Large balance change detected: {total_change} by user {account.account_number}")
        
        if suspicious_activities:
            num_activites = send_suspicious_activity_alert(suspicious_activities)
            if num_activites > 0:
                return (f"Suspicious Activity check completed. {num_activites} suspicious activites detected and reported ")
            else:
                return (f"Suspicious activity check completed. Activities detected but email failed to send! ")
    return "Suspicious activity check completed. No suspicious activies detected."
