...
class BankAccount(TimeStampedModel):
    ...
    class AccountCurrency(models.TextChoices):
        DOLLAR = ("us-dollar", _("US Dollar"))
        POUND_STERLING = ("pound_sterling",  _("Pound Sterling"))
        NIGERIA_NAIRA = ("nigeria-naira", _("Nigeria Naira"))
    
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="bank_accounts")
    account_number = models.CharField(_("Account Number"), max_length=20, unique=True)
    account_balance = models.DecimalField(_("Account Balance"), decimal_places=2, max_digits=10, default=0.00)
    currency = models.CharField(_("Currency"), max_length=20, choices=AccountCurrency.choices)
    account_status = models.CharField(_("Account Status"), max_length=10, choices=AccountStatus.choices, default=AccountStatus.INACTIVE)
    ...
    interest_rate = models.DecimalField(_("Interest Rate"), max_digits=5, decimal_places=4, default=0.00, help_text=_("Annual Interest rate as a decimal (e.g 0.0150 for 1.5%)"))
    ...
    def apply_daily_interest(self):
        if self.account_type == self.AccountType.SAVINGS:
            ...
            Transaction.objects.create(user=self.user, amount=interest, transaction_type=Transaction.TransactionType.INTEREST, description="Daily interest applied", receiver=self.user, status=Transaction.TransactionStatus.COMPLETED)
            return interest
        return Decimal("0.00")
...
class Transaction(TimeStampedModel):
    ...
    receiver_account = models.ForeignKey(BankAccount, on_delete=models.SET_NULL, null=True, related_name="received_transactions")
    sender_account = models.ForeignKey(BankAccount, on_delete=models.SET_NULL, null=True, related_name="sent_transactions",)
    status = models.CharField(choices=TransactionStatus.choices, max_length=20, default=TransactionStatus.PENDING)
