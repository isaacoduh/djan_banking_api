...
from .emails import (send_full_activation_email, send_deposit_email, send_withdrawal_email, send_transfer_email, send_transfer_otp_email)
from .models import BankAccount, Transaction
from decimal import Decimal
from .serializers import (AccountVerificationSerializer, CustomerInfoSerializer, DepositSerializer, TransactionSerializer, UsernameVerificationSerializer, SecurityQuestionSerializer, OTPVerificationSerializer)
...
class AccountVerificationView(generics.UpdateAPIView):
    ...
    def update(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        if instance.kyc_verified and instance.fully_activated:
            return Response({"message": "This account has already been verified and fully activated"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if kyc_verified and not kyc_submitted:
                return Response({"error": "KYC must be submitted before it can be verified."}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({"message": "Account Verification status updated successfully", "data": self.get_serializer(instance).data
                             }
                             )
...
class DepositView(generics.CreateAPIView):
    ...
    def get(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        try:
            ...
            return Response({"error": "Account number does not exist"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def create(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        try:
            ...
            send_deposit_email(user=account.user, user_email=account.user.email, amount=amount, currency=account.currency, new_balance=account.account_balance, account_number=account.account_number)
            return Response({"message": f"Successfully deposited {amount} to account "
                             f"{account.account_number}",
                             "new_balance": str(account.account_balance)}, status=status.HTTP_200_OK)
        except Exception as e:
            logger.info(f"Error during deposit: {str(e)}")
            return Response({"error": "An error occured during the deposit"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def create(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        try:
            ...
            if not (account.fully_activated and account.kyc_verified):
                return Response({"error": "Your account is fully verified. Please complete the verification process. "}, status=status.HTTP_403_FORBIDDEN)
        except BankAccount.DoesNotExist:
            return Response({"error": "YOu are not authorized to withdraw from this account"}, status=status.HTTP_403_FORBIDDEN)
        ...
        return Response({"message": "Withdrawal Initiated. Please verify username to complete the withdrawal.", "next_step": "Verify your username to complete withdrawal."}, status=status.HTTP_200_OK)
...
class VerifySecurityQuestionView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            return Response({"message": "Security Question Verified. An Otp has been sent to your email", "next_step": "verify_otp"}, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class VerifyOTPView(generics.CreateAPIView):
    ...
    def process_transfer(self, request)-> Response:
        ...
        logger.info(f"Transfer of {amount} made from account {sender_account.account_number} to {receiver_account.account_number}")
        
        return Response(TransactionSerializer(transfer_transaction).data, status=status.HTTP_201_CREATED)
...
class TransactionListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if account_number:
            try:
                ...
                queryset = queryset.filter(Q(sender_acccount=account) | Q(receiver_account=account))
            except BankAccount.DoesNotExist:
    ...
    def list(self, request, *args, **kwargs) -> Response:
        ...
        account_number = request.query_params.get("account_number")
        if account_number:
            logger.info(f"User {request.user.email} successfully retrieved transactions for account: {account_number}")
        else:
            logger.info(f"User {request.user.email} retrieved transactions (all accounts)")
...
class TransactionPDFView(APIView):
    ...
    def post(self, request) -> Response:
        ...
        account_number = request.data.get("acount_number") or request.query_params.get("account_number")

        if not end_date:
            end_date = timezone.now().date().isoformat()
        
        if not start_date:
            start_date = ((parser.parse(end_date) - timezone.timedelta(days=30)).date().isoformat())
        ...
        return Response(
            {
                "message": "Your transaction history pdf is being generated and will be sent to your email shortly", "email": user.email
