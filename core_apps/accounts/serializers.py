...
class AccountVerificationSerializer(serializers.ModelSerializer):
    class Meta:
        model = BankAccount
        fields = ['kyc_submitted', 'kyc_verified','verification_date', 'verification_notes', 'fully_activated', 'account_status']
    ...
    def validate(self, data: dict) -> dict:
        ...
        if kyc_verified:
            if not verification_date:
                raise serializers.ValidationError(_("Verification date is required when verifying an account."))
            if not verification_notes:
                raise serializers.ValidationError(_("Verification notes are required when verifying an account."))
            
            if kyc_submitted and not all([kyc_verified, verification_date, verification_notes]):
                raise serializers.ValidationError(_("All Verification fields (KYC verified, verification date and notes) must be provided when KYC is submitted"))
...
class CustomerInfoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = BankAccount
        fields = ["account_number", "full_name", "email", "photo_url", "account_balance", "account_type","currency"]
...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Transaction
        fields = ["id", "amount", "description","status","transaction_type","created_at","sender","receiver","sender_account","receiver_account"]
    ...
    def to_representation(self, instance: Transaction) -> str:
        ...
        representation["sender_account"] = (instance.sender_account.account_number if instance.sender_account else None)
        representation["receiver_account"] = (instance.receiver_account.account_number if instance.receiver_account else None)
        return representation
    ...
    def validate(self, data):
        ...
        try:
            if transaction_type == Transaction.TransactionType.WITHDRAWAL:
                ...
                account = BankAccount.objects.get(account_number=receiver_account_number)
                ...
                if sender_account.currency != receiver_account.currency:
                    raise serializers.ValidationError("Transfers are only allowed between accounts with the same currency")
...
class UsernameVerificationSerializer(serializers.Serializer):
    ...
    def validate_username(self, value) -> dict:
        user = self.context["request"].user
