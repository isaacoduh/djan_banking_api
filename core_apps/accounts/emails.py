...
def send_deposit_email(user, user_email, amount, currency, new_balance, account_number) -> None:
...
def send_withdrawal_email(user, user_email, amount, currency, new_balance, account_number) -> None:
    ...
    html_email = render_to_string("emails/withdrawal_confirmation.html")
    plain_email = strip_tags(html_email)
    email = EmailMultiAlternatives(subject, plain_email, from_email, recipient_list)
    email.attach_alternative(html_email, "text/html")
    try:
...
def send_transfer_email(sender_name, sender_email, receiver_name, receiver_email, amount, currency, sender_new_balance, receiver_new_balance, sender_account_number, receiver_account_number) -> None:
    ...
    common_context = {"amount": amount, "currency": currency, "sender_account_number": sender_account_number, "receiver_account_number": receiver_account_number, "sender_name": sender_name, "receiver_name": receiver_name, "site_name": settings.SITE_NAME}
    sender_context = {**common_context, "user": sender_name, "is_sender": True, "new_balance": sender_new_balance}
    ...
    receiver_context = {**common_context, "user": receiver_email, "is_sender": False, "new_balance": receiver_new_balance}
    ...
    try:
        ...
        receiver_email.obj.send()
        logger.info(
            f"Transfer notification email sent to sender: {sender_email} and receiver: "
            f"{receiver_email}"
        )
    except Exception as e:
        logger.error(f"Failed to send transfer notification email. Error: {str(e)}")

def send_transfer_otp_email(email, otp) -> None:
    subject = _("Your OTP for Transfer Authorization")
    from_email = settings.DEFAULT_FROM_EMAIL
    recipient_list = [email]
    context = {"otp": otp, "expiry_time": settings.OTP_EXPIRATION, "site_name": settings.SITE_NAME }
...
def send_suspicious_activity_alert(suspicious_activites):
    subject = _("Suspicious Activity Alert")
    from_email = settings.DEFAULT_FROM_EMAIL
    recipient_list = [settings.ADMIN_EMAIL]

    context = {"suspicious_activites" : suspicious_activites, "site_name": settings.SITE_NAME}
    ...
    try:
        email.send()
        logger.info(f"Suspicious activity alert sent successfully to: {settings.ADMIN_EMAIL}")
        return len(suspicious_activites)
    except Exception as e:
        logger.error(f"Failed to send suspicious activity alert to {settings.ADMIN_EMAIL}. Error: {str(e)}")
        return 0
